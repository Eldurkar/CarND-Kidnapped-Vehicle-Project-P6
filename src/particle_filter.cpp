...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  // Loop through particles to get position and orientation of each particle
  for (int i = 0; i < num_particles; i++) {

    // get the particle position and orientation
    double p_x = particles[i].x;
    double p_y = particles[i].y;
    double p_theta = particles[i].theta;

    // create a vector for the landmark locations predicted to be within sensor range of the particle
    ...
    for (unsigned int j = 0; j < transformed_os.size(); j++) {
      ...
      // calculate weight for this observation with multivariate Gaussian distribution
      double sigma_x = std_landmark[0];
      double sigma_y = std_landmark[1];
      double obs_w = ( 1/(2*M_PI*sigma_x*sigma_y)) * exp( -( pow(pr_x-o_x,2)/(2*pow(sigma_x, 2)) + (pow(pr_y-o_y,2)/(2*pow(sigma_y, 2))) ) );
